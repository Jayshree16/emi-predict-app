...
# # Page Config
# # ------------------------------------------
# st.set_page_config(page_title="Eligibility Predictor", page_icon="🚀", layout="wide")

# # ------------------------------------------
# # Load Model + Label Encoder + Regression Model
# # ------------------------------------------
# clf_model = joblib.load("models/best_classification_model_new.joblib")
# label_encoder = joblib.load("models/label_encoder.joblib")
# reg_model = joblib.load("models/best_regression_model_new.joblib")

# MODEL_FEATURES = clf_model.feature_names_in_.tolist()   # length = 80
...
#     monthly_salary = st.number_input("Monthly Salary", 0, 2000000, 50000)
#     years_of_employment = st.number_input("Years of Employment", 0, 40, 1)
#     monthly_rent = st.number_input("Monthly Rent", 0, 200000, 8000)

# with c2:
#     family_size = st.number_input("Family Size", 1, 20, 3)
#     dependents = st.number_input("Dependents", 0, 10, 1)
#     groceries_utilities = st.number_input("Groceries & Utilities", 0, 200000, 6000)
#     other_monthly_expenses = st.number_input("Other Monthly Expenses", 0, 200000, 4000)

# with c3:
#     credit_score = st.number_input("Credit Score", 300, 900, 700)
#     bank_balance = st.number_input("Bank Balance", 0, 10000000, 20000)
#     emergency_fund = st.number_input("Emergency Fund (₹)", 0, 10000000, 0)
#     current_emi_amount = st.number_input("Existing EMI (₹)", 0, 200000, 0)

# # Hidden: we fix all categorical fields to safe defaults
# DEFAULT_CATEGORICALS = {col: 0 for col in MODEL_FEATURES if "_" in col}

# # ------------------------------------------
# # Engineered Feature Builder
# # ------------------------------------------
# def build_feature_row():
#     """Builds a dict mapped exactly to MODEL_FEATURES in correct order."""
#     row = {col: 0 for col in MODEL_FEATURES}

#     # Raw fields
#     row['age'] = age
#     row['monthly_salary'] = monthly_salary
#     row['years_of_employment'] = years_of_employment
#     row['monthly_rent'] = monthly_rent
#     row['family_size'] = family_size
#     row['dependents'] = dependents
#     row['groceries_utilities'] = groceries_utilities
#     row['other_monthly_expenses'] = other_monthly_expenses
#     row['current_emi_amount'] = current_emi_amount
#     row['credit_score'] = credit_score
#     row['bank_balance'] = bank_balance
#     row['emergency_fund'] = emergency_fund

#     # Missing raw numeric fields
#     row['school_fees'] = 0
#     row['college_fees'] = 0
#     row['travel_expenses'] = 0
#     row['requested_amount'] = 0
#     row['requested_tenure'] = 0
#     row['max_monthly_emi'] = 0

#     # Derived features
#     total_exp = groceries_utilities + other_monthly_expenses
#     row['total_expenses'] = total_exp

#     eps = 1e-6
#     row['debt_to_income'] = current_emi_amount / (monthly_salary + eps)
#     row['expense_to_income'] = total_exp / (monthly_salary + eps)
#     row['affordability_ratio'] = (
#         (monthly_salary - total_exp - current_emi_amount) / (monthly_salary + eps)
#     )

#     row['credit_risk_score'] = credit_score / 850
#     row['employment_stability'] = years_of_employment / (years_of_employment + 1)
#     row['bank_balance_norm'] = bank_balance / (bank_balance + 1)

#     row['financial_stability_index'] = (
#         0.5 * row['credit_risk_score'] +
#         0.3 * row['employment_stability'] +
#         0.2 * row['bank_balance_norm']
#     )

#     # Interaction features
#     row['income_x_credit'] = monthly_salary * row['credit_risk_score']
#     row['income_x_stability'] = monthly_salary * row['employment_stability']
#     row['debt_x_expense'] = row['debt_to_income'] * row['expense_to_income']

#     # Fill categorical dummies with safe defaults (0)
#     for col in DEFAULT_CATEGORICALS:
#         row[col] = 0

#     # Return row ordered to match model
#     ordered = [row[col] for col in MODEL_FEATURES]
...
#     cls_pred = clf_model.predict(X)[0]
#     label = label_encoder.inverse_transform([cls_pred])[0]

#     # Regression prediction
#     emi_safe = reg_model.predict(X)[0]

#     st.success(f"Eligibility Status: **{label}**")
#     st.info(f"Recommended Safe EMI: **₹{emi_safe:,.0f}**")
